...
namespace ProfileService.Infrastructure.Pdf
{
    ...
    public class PdfService : IPdfService
    {
        ...
        public PdfService(IConverter converter, IHtmlRenderer renderer)
        {
            _converter = converter;
            _renderer = renderer;
        }

        public async Task<byte[]> GenerateResumePdf(ResumeDto resume)
        {
            string html = await _renderer.RenderResumeAsync(resume);
    ...
    public class HtmlRenderer : IHtmlRenderer
    {
        ...
        public HtmlRenderer(IWebHostEnvironment env) => _env = env;

        public async Task<string> RenderResumeAsync(ResumeDto resume)
        {
            string path = Path.Combine(_env.ContentRootPath, "Templates", "resume.html");
            string template = await File.ReadAllTextAsync(path);

            // Perform replacements
            // (can also use a templating engine like RazorLight)
            var html = template
                .Replace("{{FullName}}", resume.FullName)
                .Replace("{{Email}}", resume.Email)
                .Replace("{{Phone}}", resume.Phone);
            // Build sections (work/skills/languages)…
